 ...
 * 
 *  Thu 19 Nov 2009 06:17:41 PM EET changes by aisoftware.ro
 */
...
$.Autocompleter = function(input, options) {
		...
		function findValueCallback(q, data) {
			...
			if( data && data.length ) {
				for (var i=0; i < data.length; i++) {
					if(data[i].name && data[i].name.toLowerCase() == q.toLowerCase() ) {
						result = data[i];
						break;
					}
				}
	...
	function receiveData(q, data) {
		if ( data && data.length && hasFocus ) {
			...
			if(options.formatResult) autoFill(q, options.formatResult(data[0]));
			select.show();
		} else {
			hideResultsNow();
		}
...
$.Autocompleter.Select = function (options, input, select, config) {
	...
	function fillList() {
		...
		if(options.selectFirst){if(options.formatResult) options.formatResult(data[0]);
